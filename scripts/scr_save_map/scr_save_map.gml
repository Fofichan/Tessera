...
// @param (real) map number - the number of the map we're saving (this is the entry in the battle map list)
// @param freall grid_to_copy_from - what's the name of the grid that we just made the map in?

/*
We'11 make a temporary grid, turn the lists into a string for every cell and save that string into the cell of the temp grid then we convert the temp grid into a string and save that string to battle map list. We can then destroy the temp grid
*/
// grid_to_copy_from = argument1;
...
for(var yy = 0; yy < grid_height; yy ++) {
	for (var xx = 0; xx < grid_width; xx ++){
		...
		//Add string to grid to write_to
